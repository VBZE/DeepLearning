...
class Multi_Sys(nn.Module):
    ...
    def forward_features(self, x):
        x = self.patch_embed(x)  # [16, 3136, 96]
        if self.ape:
        ...
        for layer in self.layers:
            # [16, 3136, 96] [16, 784, 192] [16, 196, 384] [16, 49, 768]
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):  # inx: 0,1,2,3
            # [16, 49, 768] [16, 196, 384] [16, 784, 192] [16, 3136, 96]
            # 49: 7x7, 196:14*14, 784:28*28, 3136: 56*56
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            x = self.up(x)  # [16, 50176, 96]
            x = x.view(B, 4 * H, 4 * W, -1)  # [16, 224, 224, 96]
            x = x.permute(0, 3, 1, 2)  # B,C,H,W [16, 96, 224, 224]
            x = self.output(x)  # [16, 1000, 224, 224]
        return x

    def forward(self, x):

        patch1 = PatchPart(img_size=56)
        patch2 = PatchPart(img_size=28)
        patch3 = PatchPart(img_size=14)
        patch4 = PatchPart(img_size=7)

        y0 = self.pre(x)  # [16, 3, 112, 112]

        y1 = self.layer1(y0)  # [16, 96, 56, 56]
        y2 = self.layer2(y1)
        y3 = self.layer3(y2)
        y4 = self.layer4(y3)
        ...
        for index in range(len(y)):
            x_downsample[index] = x_downsample[index] + y[index]
